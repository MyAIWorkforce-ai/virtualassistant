...
export default function UseCase({ title, topBox, useCases }: UseCaseProps) {
  const icons = [
    "/industries-images/reuse-icons/uc-one.png",
    "/industries-images/reuse-icons/uc-two.png",
    "/industries-images/reuse-icons/uc-three.png",
    "/industries-images/reuse-icons/uc-four.png",
  ];
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 max-w-5xl mx-auto">
        {useCases.map((item, index) => {
          ...
          let bgClass = "";
        if (index === 0) bgClass = "bg-[#3B82F6]/10";
  else if (index === 1) bgClass = "bg-[#10B981]/10"; 
  else if (index === 2) bgClass = "bg-[#F5B80B]/10"; 
  else if (index === 3) bgClass = "bg-[#D9B4FF]/10"; 
          ...
          return (
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 250, damping: 20 }}
              className={`w-full h-auto rounded-xl text-black shadow-md ${bgClass} transition-transform flex flex-col justify-between p-4`}
            >
              <div className="flex items-center gap-3 mb-4">
                <Image
                  ...
                  priority
                />
                <h3 className="text-base text-[#000000] font-semibold">{item.title}</h3>
              </div>
